...
void Window::draw(Adafruit_ILI9341& tft)
{
    ...
    tft.drawLine(closeBoxX + 2, closeBoxY + 2, closeBoxX + 8, closeBoxY + 8, ILI9341_WHITE);
    tft.drawLine(closeBoxX + 8, closeBoxY + 2, closeBoxX + 2, closeBoxY + 8, ILI9341_WHITE);
}
...
void Window::onTouch(int16_t tx, int16_t ty)
{
    ...
    if (tx >= closeBoxX && tx <= closeBoxX + 10 && ty >= closeBoxY && ty <= closeBoxY + 10)
    {
        is_dragging = false;
